...
from models.utils.helper_methods import convert_utc_to_local, get_day_number
...
class Period(Base):
    ...
    def set_standings(self) -> bool:
        ...
        # get the tournament to determine if it is a major or not
        tournament = db.tournaments.find_one({"_id": ObjectId(self.TournamentId)})
        ...
        if league_settings["Game"] == "Matchplay":
            return False
        ...
        if team_results:
            ...
            for placing, team_result in enumerate(sorted_team_results, start=1):
                ...
                points_per_placing_arr = league_settings["PointsPerPlacing"] if not tournament["Major"] else league_settings["MajorScorePerPlacing"]

                # Check the array for the number of places awarded points
                num_of_scoring_places = len(points_per_placing_arr)

                if period_over and placing <= num_of_scoring_places:
